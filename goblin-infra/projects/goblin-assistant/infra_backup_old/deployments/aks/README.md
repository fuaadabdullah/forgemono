...
2. Update the `image` field in `manifests/deployment.yaml` to `${{ env.IMAGE }}` or use the workflow to replace it.
3. Copy the workflow into `.github/workflows/deploy-aks.yml` and customize resource names.
