...
const FASTAPI_BASE = import.meta.env.VITE_FASTAPI_URL || import.meta.env.VITE_API_URL || 'https://goblin-assistant-backend.onrender.com';
...
export class FastApiRuntimeClient implements RuntimeClient {
  ...
  async getHistory(_goblin: string, _limit = 10): Promise<MemoryEntry[]> {
    return trackApiCall(
      async () => {
        const response = await api.get<MemoryEntry[]>(`/goblins/${_goblin}/history?limit=${_limit}`);
        return response.data || [];
      },
