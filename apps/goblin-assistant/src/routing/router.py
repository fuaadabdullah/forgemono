...
print("DEBUG: Routing module loading...")
...
print(f"DEBUG: Loaded {len(PROVIDERS)} providers: {list(PROVIDERS.keys())}")
...
async def route_task(
    ...
    for pid, p in PROVIDERS.items():
        print(f"DEBUG: Checking provider {pid}")
        if task_type not in p.get("capabilities", []):
            print(f"DEBUG: {pid} doesn't support {task_type}")
            continue
        ...
        print(f"DEBUG: Adding {pid} to candidates")
