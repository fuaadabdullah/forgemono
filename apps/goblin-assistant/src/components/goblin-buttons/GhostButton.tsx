...
export function GhostButton({ children, onClick, disabled = false, variant = 'primary', className = '', ...props }: BaseButtonProps & { variant?: GhostVariant }) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} className={`${getButtonClasses('ghost', `${colorClasses[variant]} ${className}`)}`} {...props}>
      {children}
    </button>
  );
}
