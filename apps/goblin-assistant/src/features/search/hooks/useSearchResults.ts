...
import { fetchCollections, searchCollectionById, searchCollectionByName } from '../api';
...
export const useSearchResults = (): SearchState => {
  ...
  const [collectionsData, setCollectionsData] = useState<any[] | undefined>(undefined);
  ...
  const handleSearch = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        ...
        const resultsById = await searchCollectionById(collectionId, query.trim(), 20);
    ...
    [collectionsData, query, selectedCollection]
  );
