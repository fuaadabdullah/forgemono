...
export const searchCollectionById = async (
  collectionId: number,
  query: string,
  limit = 20
): Promise<SearchResult[]> => {
  try {
    const response = await apiClient.searchDocuments(collectionId, query, limit);
    return (response as any)?.results || [];
  } catch (error) {
    throw new UiError(
      {
        code: 'SEARCH_COLLECTION_FAILED',
        userMessage: 'Search could not run. Please try again.',
      },
      error
    );
  }
