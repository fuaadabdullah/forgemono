...
async def handle_autoscaling_and_emergency_routing(
    ...
    autoscaling_result = await autoscaling_service._check_autoscaling(
    ...
    if fallback_level == FallbackLevel.CHEAP_MODEL:
        ...
        requirements["model"] = autoscaling_service.cheap_fallback_model
...
async def find_fast_local_model(
    ...
    def _sync_find_provider():
        ...
        return (
            ...
            .filter(Provider.name == "goblin-ollama-server", Provider.is_active)
            .first()
        )
    ...
    try:
        ...
        adapter = adapters["goblin-ollama-server"](api_key, provider.base_url)
